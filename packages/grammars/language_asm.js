...
// Required Modules
//-----------------------------------------------------------------------------------------------//
const { Range, ParserBase, TokenType, Token, StatementType, Statement, ParserHelper, ParserIterator, CharCode } = require('grammars/language_base');
...
class AsmParser extends ParserBase {
    ...
    isAssemblerOpcode(token) {
        if (token._range.length != 3) return false;
    ...
    lexer(tokenOffset, tokenCount) {
        ...
        if (tokenType == TokenType.Comment) {
            ...
            if (!this.isAssemblerOpcode(token)) { // ignore ASM
