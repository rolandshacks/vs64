...
import { SpritePadMediaFile, SpriteMateMediaFile } from "./spritefile.js";
...
//import { SidAnalyzer } from "./sidanalyzer";
...
class MediaView extends WebView {
    constructor(app) {
        ...
        let min_scale = MIN_SCALE;
        let max_scale = MAX_SCALE;

        this._virtual_machine = null;

        {
...

            {
                ...
                /*
                ui.dataGrid = $$("idDataGrid");
                if (ui.dataGrid) {
                    ui.dataGrid.element.rowsData = [
                        { item1: "aaa", item2: "bbb" },
                        { item1: "aaa", item2: "bbb" },
                        { item1: "aaa", item2: "bbb" }
                    ];
        ...
        this.setScale(DEFAULT_SCALE, min_scale, max_scale);
    }
    ...
    onMedia(media) {
        if (this.ui) {
            this.ui.slider.visible = (media.mediaType != MediaType.SID);
        }
    }
    ...
    getMetrics(multiColor) {
        ...
        metrics.scrollbarSize = this._options.scrollbarSize;
    ...
    showCharset() {
        ...
        const scrollbarSizeX = metrics.scrollbarSize.width + 1;

        // TODO: get padding
        // getComputedStyle(document.querySelector(':root')).getPropertyValue("--container-paddding")
        const paddingX = ROOT_X_PADDING;

        let contentCols = Math.trunc((viewSizeX - paddingX) / (metrics.charWidth + metrics.charPaddingX));
        let contentRows = Math.trunc((count + contentCols - 1) / contentCols);
        ...
        if (null != media.mapData) {
            start_x = Math.max(0, Math.trunc((canvasWidth - mapDisplayWidth) / 2));
            ofs_x = start_x;
    ...
    showSid() {
        ...
        if (!ENABLE_SID_ANALYZER) return;

        const media = this.media;
        if (null == media || media.mediaType != MediaType.SID) return;

        const analyzer = new SidAnalyzer();
        analyzer.analyze(this.media);

        const ui = this.ui;
        const viewSize = this.getViewSize();
        const metrics = this.getMetrics();
        analyzer.render(ui, viewSize, metrics)
    }
